...
Subsequently, the experiments for which a plot shall be created can be selected. For each selected experiment one plot is created with the corresponding evolution of the cell state proportions and the predicted evolution estimated by CellTrans.
...
Several other options are conveniently asked for in dialog windows, e.g. the range of the y-axis and the cell states you would like to plot. Finally, you can choose the directory of the plot. In our example, we get for experiment 1 the following plot which is saved as `Plot_1.pdf` in the selected directory.

![](C:\Users\Thomas\Dropbox\TU\R\CellTrans\inst\doc\images\Plot_1.png)  
