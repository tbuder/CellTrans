#' Allows to plot data  and predictions in pdf format.
#' 
#' This function plots the predictions of CellTrans and the experimental data. The variable input contains the read data from readExperimentalData(). The user of CellTrans can choose in a dialog box the number of the experiment for which the plot shall be created.
...
initExp <- as.integer(dlgList(title="Select experiments to plot.",multiple=TRUE, choices=c(1:input$cellnr))$res)
statestoplot <- as.integer(dlgList(title="Which cell states should be plotted?",multiple=TRUE, choices=c(1:input$cellnr))$res)
